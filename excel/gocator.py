...
def acc_interval(anal_path: str, acc_intervals: list, use_individual_interval: bool = False):
    ...
    profile_paths = []
    for gocator_path in gocator_paths:
        positions = glob.glob(gocator_path + r'\*')
        for pos in positions:
            if os.path.basename(pos).lower() in gocator_path.lower():
                profile_paths.append(pos)

    for gocator_index, profile_path in enumerate(profile_paths):
        # Rawfile path 지정
        anal_name = os.path.basename(os.path.dirname(profile_path))
        sensor_location = os.path.basename(profile_path)
        rawfile_path = os.path.join(result_path, (anal_name + '_' + sensor_location + '.raw'))
        print(fr"{anal_name + '_' + sensor_location} 진행중.... {gocator_index + 1}/{len(profile_paths)}")

        csv_files = glob.glob(profile_path + r'\*Profile\*.csv')
        imu_file = glob.glob(imu_paths[gocator_index] + fr'\*{os.path.basename(profile_path)}.txt')
        if not csv_files:
        ...
        for csv_file in csv_files:
            csv_df = csv_df.append(pd.read_csv(csv_file, header=None, usecols=[3200]))
...
if __name__ == "__main__":
    ...
    intervals = [2.777, 5.555, 8.333, 11.111, 13.888] * 3
    # intervals = [2.777, 13.888]
    input_path = r'S:\2008_3D 노면 검사시스템 개발(도공+중기부 구매조건부)\20221206_gocator교정데이터'
